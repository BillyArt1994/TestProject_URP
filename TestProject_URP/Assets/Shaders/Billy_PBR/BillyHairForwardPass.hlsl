...
float4 frag (Varyings input , half face : VFACE) : SV_Target
{
    ...
    float ao = tex2D(_AO_Root,input.uv1);

    float rootFlow = saturate(Remap(flowMap0.b,0,1,0,_TipVariation*2));
    float EdgeSmooth = _Edge <= _EdgeSmooth ? _Edge -0.01 : _EdgeSmooth;
    rootFlow = SmoothStep(_Edge,EdgeSmooth ,flowMap0.b);
  

    albedo.xyz =lerp(_RootColor,albedo.xyz,rootFlow );

    //return  albedo.xyzz;
    float t1 = 0;
    float t2 = flowMap2.z *2.0 -1.0;
    half faceFlag = face == 0 ? -1: 1;
    float3 T = float3(0,1,0.0);
    T.xy = flowMap0.xy < 0.001 ? T.xy:flowMap0.xy;//y < 0.1 ? 1.0:0.0;
    T.xy = normalize(T.xy) ;
    T.z = 0;
    //return T.xyzz;
    ...
    return float4(col.xyz,flowMap0.a);
}
