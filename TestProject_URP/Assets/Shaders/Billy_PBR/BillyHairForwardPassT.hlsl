...
float4 frag (Varyings input , half face : VFACE) : SV_Target
{
    ...
    float rootFlow = SmoothStep(_Edge,EdgeSmooth ,flowMap0.b);
    albedo.xyz =lerp(_RootColor,albedo.xyz, saturate(rootFlow* flowMap0.g *_TipVariation*5) );
    ...
    T.xy = lerp(T.xy,flowMap0.xy,flowMap0.a);
    T.z = 0;
    ...
    float ar = flowMap0.a - _Cutoff ;//- r;
   // flowMap0.a =  flowMap0.a/(r+0.001);
   // flowMap0.a = saturate(flowMap0.a);

    float alpha = saturate(flowMap0.a  )  ;
