...
#include "BillySkinInput.hlsl"
    ...
    float3 Billy_Skin_Lighting(BillyBRDFData brdfData,float3 normal,float3 viewDir,float curvature,float thickness,float3 vertexNormal,Light light)
    {
       ...
       // return transmittance.xxxx;
        ...
        float3 kd = (1-F);
        float3 deepScatter= _TranslucencyColor*brdfData.diffuse*transmittance*light.color;
        

        float3 directdiffuse =  brdfData.diffuse*kd*diffuseBrdf*light.distanceAttenuation *light.color*shadow+deepScatter;
        float3 directspecular = (D1*1.5+D2*0.5)*F*G*PI*NdotL*light.distanceAttenuation *light.color*shadow;
        //return diffuseBrdf;
        directspecular = KS_Skin_Specular(normal,light.direction,viewDir,brdfData.roughness,1.0,_BeckManTex);
        //return directSpec;

       //half3 fakeAmbientColor = backContrib * _fakeAmbientIntensity*_fakeAmbientColor;
