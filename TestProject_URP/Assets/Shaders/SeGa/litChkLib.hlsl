...
//		float3 _WorldSpaceCameraPos;	float4 _ProjectionParams;	float4 _ScreenParams;	float4 _ZBufferParams;	float4 unity_OrthoParams;
//CBUFFER_END
...
LightSTB GetMainLight(CameraSTB cam)
{
    ...
    output.color = _MainLightColor.rgb *PI;
 #else
 ...
 #endif
	output.atten = atten;
    output.dirWS = _MainLightPosition.xyz;
#else
	output.color = _MainLightColor.rgb;
	output.atten = 0;
	output.dirWS = _MainLightPosition.xyz;//float3(0,0,1);
#endif
	return output;
}
