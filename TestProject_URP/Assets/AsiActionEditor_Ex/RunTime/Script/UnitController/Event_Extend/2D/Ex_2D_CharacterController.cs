...
{
    public class Ex_2D_CharacterController : ActionLogics
    {
private bool mIsMove = false;
        private bool mChangeGround_last = false;
        private Rigidbody2D mCharacter;

        public Vector2 CharacterVelocity;//玩家位移速度
        public Vector2 CharacterMove;//玩家位移
        ...
        public override void Start(ActionStateMachine _actionState)
        {
            ...
            {
                if (_actionState.TryGetComponent(out Rigidbody2D _characterController))
                {
                    //数据初始化
                ...
                {
                    mIsMove = false;
                    EngineDebug.LogError($"角色无法位移 CharacterController 获取失败！！");
                }
            }
        }
        ...
        public override void Update(ActionStateMachine _actionState, float _delaTime)
        {
            if (mIsMove)
            {
                ...
                CharacterMove /= _delaTime;
                mCharacter.velocity = (CharacterVelocity + CharacterMove);
