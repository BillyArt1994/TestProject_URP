...
{
    public class SubstancePainterLookupPass : ScriptableRenderPass
    {
        ...
        public static SubstancePainterLookupPass m_Instance;

        bool m_AllowColorGradingACESHDR = true;

        public SubstancePainterLookupPass(RenderPassEvent evt, PostProcessData data)
        {
        ...
        private static void ExecutePass(ScriptableRenderContext context, PassData passData, ref RenderingData renderingData, RTHandle internalLutTarget)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.ColorGradingLUT)))
            {
                ...
                var substancePainterLookup = stack.GetComponent<SubstancePainterLookup>();
              ...
              //  var rtTem = new RenderTexture(substancePainterLookup.lookuptexture.value.width, substancePainterLookup.lookuptexture.value.height);
               //  = RenderTexture.active;
                

               //var rtTemp = RenderTexture.GetTemporary(substancePainterLookup.lookuptexture.value.width, substancePainterLookup.lookuptexture.value.height,0,RenderTextureFormat.ARGB32);
               //RenderTexture.active = rtTemp;
               //Graphics.Blit(substancePainterLookup.lookuptexture.value, rtTemp);
               //
               //var rtDes = RenderTexture.GetTemporary(substancePainterLookup.lookuptexture.value.width, substancePainterLookup.lookuptexture.value.height, 0, RenderTextureFormat.ARGBFloat);
               //RenderTexture.active = rtDes;
               //// Render the lut
               //Blitter.BlitCameraTexture(cmd, rtTemp, rtTemp, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, material, 0);
               //cmd.Blit(rtTemp, rtDes, material);
        ...
        void SaveLookUpLut(RTHandle rt_pointer,string filePath)
        {
            RenderTexture.active = rt_pointer.rt;
            var tex2D = new Texture2D(rt_pointer.rt.width, rt_pointer.rt.height, TextureFormat.RGBAFloat, false);
            tex2D.ReadPixels(new Rect(0, 0, rt_pointer.rt.width, rt_pointer.rt.height), 0, 0);
            tex2D.Apply();
